<article class="markdown-body doc">

# <p align=center>使用指南</p>
...
> [!NOTE]说明
> Android 平台通过安装 Android 版专用客户端实现汉化

> [!TIP]提示
> **[Kuyo加速器](https://kuyo.cc)** 已同步本项目的汉化文本，如有需要可前往下载

> [!WARNING]注意
> 除专用客户端外，目前均不支持开启 `主线中配`、`图像视频` 汉化及 `登录加速` 等功能

> [!IMPORTANT]重要
> 共存版客户端可以与官方客户端共存，独占版客户端必须卸载官方客户端，但对模拟器兼容性更好
...
> [!NOTE]说明
> iOS 平台通过侧载 iOS 版专用客户端实现汉化

> [!TIP]提示
> 建议使用 **[Impactor](https://impactor.khcrysalis.dev)**、**[AltStore](https://altstore.io)** 或 **[SideStore](https://sidestore.io)** 等工具进行自签，或使用免签版客户端

> [!WARNING]注意
> 免签版客户端签名所用的证书可能随时被吊销，且签名改变后无法覆盖安装，建议优先通过自签侧载

> [!IMPORTANT]重要
> 自签版安装包必须经过签名才能安装，免签版安装包在 <kbd>设置</kbd>-<kbd>通用</kbd>-<kbd>VPN与设备管理</kbd> 中信任开发者即可使用
...
> [!NOTE]说明
> Windows 平台通过使用 Windows 版专用启动器实现汉化

> [!TIP]提示
> 专用启动器可以覆盖于官方启动器使用，或者独立于官方启动器使用，两者可以共用游戏资源文件

> [!WARNING]注意
> 如遇汉化无法生效，请尝试在系统托盘内彻底退出并重启启动器，然后在启动器设置内点击 <kbd>完整修复</kbd>

> [!IMPORTANT]重要
> 专用启动器下载后解压即可使用，建议覆盖官方启动器文件或移动到固定目录并创建快捷方式
...
> [!IMPORTANT]重要
> 由于 `PlayCover` 会修改包内文件，通过其安装的客户端无法通过反作弊校验，并会导致客户端闪退
